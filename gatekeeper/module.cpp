...
using arpi::SoftGateKeeperDevice;

struct arpi_gatekeeper_device {
    gatekeeper_device device;
    SoftGateKeeperDevice *s_gatekeeper;
};

static arpi_gatekeeper_device s_device;
...
static int enroll(const struct gatekeeper_device *dev __unused, uint32_t uid,
    ...
    SoftGateKeeperDevice *s_gatekeeper = ((arpi_gatekeeper_device*)(dev))->s_gatekeeper;
...
static int arpi_gatekeeper_open(const hw_module_t *module, const char *name,
        hw_device_t **device) {
